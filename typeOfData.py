...
def check_media_in_csv(df):
    """ Check if any column contains file paths referencing media files (image, video, etc.) """
    for column in df.columns:
        if df[column].apply(lambda x: isinstance(x, str) and x.endswith(image_extensions)).any():
            return 'Contains Image References'
        elif df[column].apply(lambda x: isinstance(x, str) and x.endswith(video_extensions)).any():
            return 'Contains Video References'
        elif df[column].apply(lambda x: isinstance(x, str) and x.endswith(graph_extensions)).any():
            return 'Contains Graph References'
    return 'No Media References'
...
def determine_data_type(file_path):
    try:
        ...
        if file_path.endswith(csv_extensions):
            ...
            # Check if any column in the CSV contains references to media files
            media_check = check_media_in_csv(df)
            return {
                "file": file_path,
                "data_type": data_type,
